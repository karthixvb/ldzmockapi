"""
GPU-accelerated API using PyTorch + CUDA 12
Requires: pip install -r requirements_cuda.txt
"""
...
from image_utils_gpu import composite_image_gpu, GPUImageCompositor
...
gpu_compositor = GPUImageCompositor(device='cuda', max_workers=4)
...
# Template Management APIs (same as other versions)
...
# GPU-Accelerated Image Compositing API

@app.route('/api/composite', methods=['POST'])
def composite_images():
    """GPU-accelerated composite PNG images into mockup variants"""
    try:
        data = request.json
        ...
        for idx, source in enumerate(sources):
            ...
            if source_type == 'url':
                ...
                try:
                    # Batch composite all variants using GPU
                    composited_images = gpu_compositor.composite_batch_gpu(
                        mockup_urls,
                        source_url,
                        design_areas,
                        options
...
def health_check():
    ...
    return jsonify({
        'status': 'healthy',
        'version': 'gpu',
        'device': str(gpu_compositor.device),
